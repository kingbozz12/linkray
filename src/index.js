...
/* LINKRAY_CLEAN_CORE_MENU_START */
app.use(async function linkrayCleanCoreMenu(req, res, next) {
  try {
    if (req.method !== 'POST') return next();

    const update = req.body || {};
    const payload = String((typeof getCallbackPayload === 'function' ? getCallbackPayload(update) : '') || '');
    const callbackId = typeof getCallbackId === 'function' ? getCallbackId(update) : null;
    const chatId = Number((typeof getChatId === 'function' ? getChatId(update) : 0) || 0);
    const text = String((typeof getMessageText === 'function' ? getMessageText(update) : '') || '').trim();

    const isStart = text === '/start';
    const isMainMenu = payload === 'main:menu';
    ...
    function b(label, data) {
      return typeof callbackButton === 'function'
        ? callbackButton(label, data)
        : { text: label, payload: data };
    }

    function keyboard(rows) {
      return typeof inlineKeyboard === 'function' ? inlineKeyboard(rows) : rows;
    }

    const menuRows = [
      [b('🚀 Создать закуп', 'post:create')],
      [b('📊 Аналитика', 'main:analytics'), b('➕ Добавить канал', 'channel:add')],
      [b('🗂 Креативы', 'stub:creatives'), b('📋 История', 'stub:history')],
      [b('🛡 Антифрод', 'stub:antifraud'), b('👤 Профиль', 'stub:profile')],
      [b('💎 Тарифы', 'stub:tariffs')]
    ];

    async function sendCleanMenu() {
      const body =
        '━━━━━━━━━━━━━━\n' +
        '⚡ <b>LinkRay</b>\n\n' +
        '🚀 <b>Создать закуп</b> — создание и планирование постов.\n' +
        '📊 <b>Аналитика</b> — карточки, графики и отчёты.\n' +
        '➕ <b>Добавить канал</b> — подключение MAX-канала.\n\n' +
        'Остальные разделы будут подключены позже.\n' +
        '━━━━━━━━━━━━━━';
      ...
      if (chatId && typeof sendMaxMessage === 'function') {
        await sendMaxMessage({
          ...
          text: body,
          format: 'html',
          attachments: keyboard(menuRows)
        });
        return true;
      }

      if (chatId && typeof msg === 'function') {
        await msg(chatId, body, menuRows, 'html');
        return true;
      }

      if (callbackId && typeof cb === 'function') {
        await cb(callbackId, body, menuRows);
        return true;
      }
    ...
    async function sendStub(stubText) {
      const rows = [[b('⬅️ Главное меню', 'main:menu')]];

      if (callbackId && typeof cb === 'function') {
        await cb(callbackId, stubText, rows);
        return true;
      }
      ...
      if (chatId && typeof msg === 'function') {
        await msg(chatId, stubText, rows, 'html');
        return true;
      }
    ...
    if (isStart || isMainMenu) {
      await sendCleanMenu();
      return res.json({ ok: true });
    }

    if (stubTexts[payload]) {
      await sendStub(stubTexts[payload]);
      return res.json({ ok: true });
    }

    return next();
  } catch (e) {
    console.error('[LINKRAY_CLEAN_CORE_MENU]', e?.stack || e?.message || e);
    return next();
  }
});
/* LINKRAY_CLEAN_CORE_MENU_END */
