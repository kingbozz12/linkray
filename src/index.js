...
/* LR_CHANNEL_LIFECYCLE_HANDLER_V55_START */
function lrV55Rows(r) {
...
function lrV55SafeJson(v) {
  try { return JSON.stringify(v); } catch { return '{}'; }
}

function lrV55Pick(obj, path) {
  let cur = obj;
  for (const p of path) {
...
function lrV55DialogChatId(update) {
  return (
    lrV55Pick(update, ['message', 'recipient', 'chat_id']) ||
    lrV55Pick(update, ['message', 'chat_id']) ||
    lrV55Pick(update, ['recipient', 'chat_id']) ||
    lrV55Pick(update, ['chat_id']) ||
    null
  );
}

function lrV55SenderUserId(update) {
  return (
    lrV55Pick(update, ['message', 'sender', 'user_id']) ||
    lrV55Pick(update, ['sender', 'user_id']) ||
    lrV55Pick(update, ['user_id']) ||
    null
  );
}

async function lrV55EnsureTables() {
  await query(`
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS lr_channel_lifecycle_seen (
...
async function lrV55RememberDialog(update) {
  const chatId = lrV55DialogChatId(update);
  const userId = lrV55SenderUserId(update);

  if (!chatId || !userId) return;

  await lrV55EnsureTables();
...
async function lrV55LastDialogChatId(ownerId = null) {
  await lrV55EnsureTables();

  if (ownerId) {
    const r = lrV55Rows(await query(
  ...
  const latest = lrV55Rows(await query(
...
async function lrV55SendNotice(chatId, html) {
  const t = lrV55Token();
  if (!t || !chatId) return false;

  try {
    const res = await fetch(`${lrV55ApiBase()}/messages?chat_id=${encodeURIComponent(String(chatId))}`, {
      method: 'POST',
      headers: {
        Authorization: t,
        'Content-Type': 'application/json'
    ...
    if (!res.ok) {
      console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] notice failed', res.status, txt.slice(0, 500));
      return false;
    }

    console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] notice sent', String(chatId));
    return true;
  } catch (e) {
    console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] notice error', e?.message || e);
    return false;
  }
}

async function lrV55MaxGet(path) {
  const t = lrV55Token();
  ...
  try {
    const res = await fetch(`${lrV55ApiBase()}${path}`, {
      method: 'GET',
      headers: {
        Authorization: t,
...
async function lrV55BotIsAdmin(maxChatId) {
  const chat = await lrV55MaxGet(`/chats/${encodeURIComponent(String(maxChatId))}`);
  const member = await lrV55MaxGet(`/chats/${encodeURIComponent(String(maxChatId))}/members/me`);
  const info = lrV55MemberInfo(member);
...
function lrV55EventFingerprint(update, action, chatId) {
  const id =
    update.update_id ||
    update.timestamp ||
    lrV55Pick(update, ['message', 'body', 'mid']) ||
    lrV55Pick(update, ['message', 'body', 'seq']) ||
    lrV55Pick(update, ['body', 'mid']) ||
    Buffer.from(JSON.stringify(update).slice(0, 1200)).toString('base64').slice(0, 120);

  return `${action}:${chatId}:${id}`;
}

async function lrV55Once(update, action, chatId) {
  await lrV55EnsureTables();

  const fp = lrV55EventFingerprint(update, action, chatId);

  const r = lrV55Rows(await query(`
    INSERT INTO lr_channel_lifecycle_seen(fingerprint, created_at)
    VALUES($1, now())
...
function lrV55FindChannelEvents(update) {
  const out = [];
  const seen = new Set();
  const rootText = JSON.stringify(update || {}).toLowerCase();

  const rootRemove =
    rootText.includes('removed') ||
    rootText.includes('left') ||
    rootText.includes('kicked') ||
    rootText.includes('delete') ||
    rootText.includes('bot_removed') ||
    rootText.includes('chat_member_removed');
  ...
  function walk(obj, depth = 0) {
    ...
    for (const k of ['chat_id', 'chatId', 'max_chat_id', 'channel_id', 'channelId']) {
      if (obj[k] !== undefined && obj[k] !== null) add(obj[k], maybeTitle, action);
    }
...
async function lrV55UpsertChannel(update, ev, access) {
  await lrV55EnsureTables();

  const owner = lrV55SenderUserId(update);
  const title =
    lrV55ChatTitle(access.chat) ||
    ev.title ||
    `Канал ${ev.chatId}`;

  const link = lrV55ChatLink(access.chat) || null;
  ...
  return r[0] || {
    id: null,
    max_chat_id: String(ev.chatId),
    title,
    link
  };
}

async function lrV55DeleteChannel(ev) {
  await lrV55EnsureTables();

  const r = lrV55Rows(await query(
    `DELETE FROM channels WHERE max_chat_id=$1 RETURNING id, max_chat_id, title, link`,
    [String(ev.chatId)]
  ).catch(() => []));
  ...
  return r[0] || {
    id: null,
    max_chat_id: String(ev.chatId),
    title: ev.title || `Канал ${ev.chatId}`
  };
}

async function lrHandleChannelLifecycleV1(updateOrReq) {
  const update = updateOrReq?.body && typeof updateOrReq.body === 'object'
    ? updateOrReq.body
    : (updateOrReq || {});

  try {
    await lrV55RememberDialog(update);

    const events = lrV55FindChannelEvents(update);

    if (!events.length) return null;

    console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] events', lrV55SafeJson(events));

    for (const ev of events) {
      const firstTime = await lrV55Once(update, ev.action, ev.chatId);
      if (!firstTime) continue;

      const owner = lrV55SenderUserId(update);
      const noticeChatId = await lrV55LastDialogChatId(owner);

      if (ev.action === 'remove') {
        const deleted = await lrV55DeleteChannel(ev);

        await lrV55SendNotice(
          noticeChatId,
          `🗑 <b>Канал отключён от LinkRay</b>\n\n` +
          `${lrV55Esc(deleted.title || ev.title || ev.chatId)}\n\n` +
          `Канал удалён из базы LinkRay, меню публикаций, аналитики, ежедневных отчётов, антифрода и рекламных закупов.`
        );

        console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] removed', lrV55SafeJson(deleted));
        continue;
      }

      const access = await lrV55BotIsAdmin(ev.chatId);

      if (!access.ok) {
        console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] add skipped, bot not admin', lrV55SafeJson({
          chatId: ev.chatId,
      ...
      const saved = await lrV55UpsertChannel(update, ev, access);

      await lrV55SendNotice(
        noticeChatId,
        `✅ <b>Канал подключён к LinkRay</b>\n\n` +
        `${lrV55Esc(saved.title || ev.title || ev.chatId)}\n\n` +
        `Канал сохранён в базе LinkRay и будет использоваться для публикаций, отложенных постов, аналитики, ежедневных отчётов, антифрода и рекламных закупов.`
      );

      console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] added', lrV55SafeJson(saved));
    }

    return true;
  } catch (e) {
    console.log('[LR_CHANNEL_LIFECYCLE_HANDLER_V55] error', e?.stack || e?.message || e);
    return false;
  }
}
/* LR_CHANNEL_LIFECYCLE_HANDLER_V55_END */
