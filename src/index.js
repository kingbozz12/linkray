...
/* LR_OLD_BUTTONS_POST_ENTRY_V68_START */
const LR_OLD_BUTTONS_POST_ENTRY_V68_TAG = 'LR_OLD_BUTTONS_POST_ENTRY_V68';

function lr68Pick(obj, path) {
  let cur = obj;
...
function lr68ChatId(update) {
  return (
    lr68Pick(update, ['message', 'recipient', 'chat_id']) ||
    lr68Pick(update, ['message', 'chat_id']) ||
    lr68Pick(update, ['callback', 'message', 'recipient', 'chat_id']) ||
    lr68Pick(update, ['callback', 'message', 'chat_id']) ||
    lr68Pick(update, ['recipient', 'chat_id']) ||
    lr68Pick(update, ['chat_id']) ||
    null
  );
}

function lr68UserId(update) {
  return (
    lr68Pick(update, ['message', 'sender', 'user_id']) ||
    lr68Pick(update, ['callback', 'user', 'user_id']) ||
    lr68Pick(update, ['callback', 'sender', 'user_id']) ||
    lr68Pick(update, ['sender', 'user_id']) ||
    lr68Pick(update, ['user_id']) ||
    ''
  );
}

function lr68Text(update) {
  const direct =
    lr68Pick(update, ['message', 'body', 'text']) ||
    lr68Pick(update, ['message', 'text']) ||
    lr68Pick(update, ['body', 'text']) ||
    lr68Pick(update, ['text']);
  ...
  while (stack.length) {
    ...
    if (typeof cur.text === 'string' && cur.text.trim()) return cur.text.trim();
    if (cur.body && typeof cur.body.text === 'string' && cur.body.text.trim()) return cur.body.text.trim();
...
function lr68Payload(update) {
  const values = [
    lr68Pick(update, ['callback', 'payload']),
    lr68Pick(update, ['callback', 'data']),
    lr68Pick(update, ['callback', 'callback_data']),
    lr68Pick(update, ['callback', 'button', 'payload']),
    lr68Pick(update, ['callback', 'button', 'data']),
    lr68Pick(update, ['body', 'payload']),
    lr68Pick(update, ['payload']),
    lr68Pick(update, ['data']),
  ];
...
function lr68Btn(text, payload) {
  return { type: 'callback', text, payload };
}

function lr68IsPrivate(chatId) {
  return chatId && !String(chatId).startsWith('-');
}

function lr68IsCommand(text) {
  return String(text || '').trim().startsWith('/');
}

function lr68IsProbablyForward(update) {
...
async function lr68Send(chatId, text, buttons = []) {
  const token = lr68Token();
  ...
  try {
    const res = await fetch(`${lr68ApiBase()}/messages?chat_id=${encodeURIComponent(String(chatId))}`, {
      method: 'POST',
      headers: {
        Authorization: token,
    ...
    console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] send`, JSON.stringify({
      ok: res.ok,
      http: res.status,
    ...
    console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] send error`, e?.message || e);
    return false;
  }
}

async function lr68EnsureTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS lr_post_entry_v68 (
...
async function lr68Channels() {
  return lr68Rows(await query(`
...
function lr68Short(v, n = 36) {
  const s = String(v || '').trim() || 'Канал';
  return s.length > n ? s.slice(0, n - 1) + '…' : s;
}

async function lr68ShowStudio(chatId) {
  const channels = await lr68Channels();
  ...
  return lr68Send(
    ...
    `━━━━━━━━━━━━━━`,
    [
      [lr68Btn('📝 Создать пост', 'lr68:create_post')],
      [lr68Btn('📋 Посты', 'post:channels')],
      [lr68Btn('↩️ Главное меню', 'main:menu')]
    ]
  );
}

async function lr68ShowReports(chatId) {
  return lr68Send(
    chatId,
    `━━━━━━━━━━━━━━\n` +
    `📈 <b>Отчёты</b>\n\n` +
    `Статистика размещений, просмотры, CPM и рекламные отчёты.\n\n` +
    `Раздел снова подключён к главному меню.\n` +
    `━━━━━━━━━━━━━━`,
    [
      [lr68Btn('📊 Аналитика', 'main:analytics')],
      [lr68Btn('↩️ Главное меню', 'main:menu')]
    ]
  );
}

async function lr68ShowAntifraud(chatId) {
  return lr68Send(
    chatId,
    `━━━━━━━━━━━━━━\n` +
    `🛡️ <b>Антифрод</b>\n\n` +
    `Проверка качества трафика, подозрительных скачков и рекламных размещений.\n\n` +
    `Раздел снова подключён к главному меню.\n` +
    `━━━━━━━━━━━━━━`,
    [
      [lr68Btn('📊 Аналитика', 'main:analytics')],
...
async function lr68ShowChannelMenu(chatId, entryId = 0) {
  const channels = await lr68Channels();
  ...
  if (!channels.length) {
    return lr68Send(
      ...
      [
        [lr68Btn('➕ Добавить канал', 'main:add_channel')],
        [lr68Btn('↩️ Главное меню', 'main:menu')]
      ]
    );
  }
  ...
  for (const ch of channels.slice(0, 20)) {
    buttons.push([lr68Btn(`📡 ${lr68Short(ch.title)}`, `lr68:pick:${entryId}:${ch.id}`)]);
  }

  buttons.push([lr68Btn('🌐 Все каналы', `lr68:pick:${entryId}:all`)]);
  buttons.push([lr68Btn('❌ Отмена', `lr68:cancel:${entryId}`)]);
...
async function lr68OpenOldEditor(chatId, entryId, target) {
  await lr68EnsureTable();

  const channels = await lr68Channels();

  const selected = String(target) === 'all'
    ? channels
    : channels.filter((c) => String(c.id) === String(target));

  if (!selected.length) {
    return lr68Send(chatId, `⚠️ Канал не найден в базе.`, [[lr68Btn('↩️ Главное меню', 'main:menu')]]);
  }

  let entry = null;

  if (Number(entryId) > 0) {
    entry = lr68Rows(await query(
      `SELECT * FROM lr_post_entry_v68 WHERE id=$1 LIMIT 1`,
      [Number(entryId)]
    ).catch(() => []))[0] || null;
  }
  ...
  const draft = {
    channelIds: selected.map((c) => Number(c.id)).filter(Boolean),
    channels: selected.map((c) => Number(c.id)).filter(Boolean),
    text,
    format: 'html',
    markup: [],
    attachments: [],
    ...
    rawUpdate: entry?.raw || null,
    sourceUpdate: entry?.raw || null,
    originalUpdate: entry?.raw || null
  };
  ...
  try {
    if (typeof setSession === 'function') {
      await setSession(key, 'edit_post', { draft });
    }

    if (typeof sendEditorAsNew === 'function') {
      await sendEditorAsNew(chatId, key, draft);
      if (Number(entryId) > 0) await query(`DELETE FROM lr_post_entry_v68 WHERE id=$1`, [Number(entryId)]).catch(() => {});
      console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] opened old editor`, JSON.stringify({ chatId, entryId, channels: draft.channelIds.length }));
      return true;
    }

    if (typeof sendEditor === 'function') {
      await sendEditor(chatId, key, draft);
      if (Number(entryId) > 0) await query(`DELETE FROM lr_post_entry_v68 WHERE id=$1`, [Number(entryId)]).catch(() => {});
      console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] opened editor`, JSON.stringify({ chatId, entryId, channels: draft.channelIds.length }));
      return true;
    }
  } catch (e) {
    console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] editor error`, e?.stack || e?.message || e);
  }

  const list = selected.map((c) => `• ${lr68Esc(c.title || `Канал ${c.id}`)}`).join('\n');

  return lr68Send(
    chatId,
    `✅ <b>Канал выбран.</b>\n\n${list}\n\n` +
    `Текст сохранён. Откройте <b>LinkRay Studio</b> → <b>Создать пост</b>, если редактор не открылся автоматически.`,
    [[lr68Btn('🚀 LinkRay Studio', 'lr68:studio')]]
  );
}

async function lr68HandleCallback(update, chatId, payload) {
  console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] callback`, JSON.stringify({ chatId, payload }));

  if (payload === 'lr68:studio') return lr68ShowStudio(chatId);
  if (payload === 'lr68:reports') return lr68ShowReports(chatId);
  if (payload === 'lr68:antifraud') return lr68ShowAntifraud(chatId);
  if (payload === 'lr68:create_post') return lr68ShowChannelMenu(chatId, 0);

  if (payload.startsWith('lr68:pick:')) {
    const parts = payload.split(':');
    return lr68OpenOldEditor(chatId, Number(parts[2] || 0), parts[3] || '');
  }

  if (payload.startsWith('lr68:cancel:')) {
    const id = Number(payload.split(':')[2] || 0);
    if (id > 0) await query(`DELETE FROM lr_post_entry_v68 WHERE id=$1`, [id]).catch(() => {});
    return lr68Send(chatId, `❌ Создание поста отменено.`);
  }

  return false;
}

async function lr68HandleIncomingPost(update, chatId) {
  const text = lr68Text(update);
  const isForward = lr68IsProbablyForward(update);

  if (!text && !isForward) return false;
  if (lr68IsCommand(text)) return false;

  // Если старый редактор уже ждёт контент — не мешаем старому обработчику.
  try {
    if (typeof getSession === 'function') {
      const s = await getSession(String(chatId));
      const state = String(s?.state || s?.name || '').toLowerCase();
      if (state && !['main', 'idle', 'none', 'null', 'undefined'].includes(state)) {
        console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] skip because session active`, JSON.stringify({ chatId, state }));
        return false;
      }
    }
  } catch {}
  ...
  if (!channels.length) {
    console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] no channels for incoming post`, JSON.stringify({ chatId }));
    return false;
  }

  await lr68EnsureTable();

  const saved = lr68Rows(await query(
    `INSERT INTO lr_post_entry_v68(chat_id,user_id,text,raw,created_at)
     VALUES($1,$2,$3,$4::jsonb,now())
     RETURNING id`,
    [
      String(chatId),
      String(lr68UserId(update) || ''),
      text || '',
      JSON.stringify(update || {})
    ]
  ).catch((e) => {
    console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] insert error`, e?.message || e);
    return [];
  }))[0];

  const entryId = Number(saved?.id || 0);

  console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] incoming post`, JSON.stringify({
    chatId,
    entryId,
    textLen: text.length,
    isForward,
  ...
  await lr68ShowChannelMenu(chatId, entryId);
...
app.use((req, res, next) => {
  ...
  const chatId = lr68ChatId(update);

  if (!lr68IsPrivate(chatId)) return next();

  const payload = lr68Payload(update);

  if (type.includes('callback') && String(payload || '').startsWith('lr68:')) {
    Promise.resolve(lr68HandleCallback(update, chatId, payload))
      .then(() => res.status(200).json({ ok: true }))
      .catch((e) => {
        console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] callback fatal`, e?.stack || e?.message || e);
        res.status(200).json({ ok: true });
      });
    return;
  }

  if (type === 'message_created' || !type) {
    Promise.resolve(lr68HandleIncomingPost(update, chatId))
      .then((done) => {
        if (done) return res.status(200).json({ ok: true });
        return next();
      })
      .catch((e) => {
        console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] incoming fatal`, e?.stack || e?.message || e);
        return next();
      });
    return;
  }
...
console.log(`[${LR_OLD_BUTTONS_POST_ENTRY_V68_TAG}] mounted`);
/* LR_OLD_BUTTONS_POST_ENTRY_V68_END */
